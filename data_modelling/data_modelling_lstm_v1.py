...
train_fields = ['session_position','session_length','skip_2','duration',
                ...
                'acoustic_vector_6','acoustic_vector_7','skip_2_avg']
...
test_fields = ['session_position','session_length',
               ...
               'acoustic_vector_5', 'acoustic_vector_6', 'acoustic_vector_7', 'skip_2_avg']
...
for file_i in tqdm(range(0,10,1)):
    ...
    '''LSTM'''
    model = Sequential()
    model.add(LSTM(100, input_shape=(20,30),return_sequences=True))
    ...
    for line_num in range(len(y_pred)):
        ...
        for i in position_list[line_num]:
            pos = i-1
            if y_pred[line_num][pos]>=0.5:
                output += str(0)
            else:
                output += str(1)
        output += '\n'

    output_path = 'output_1231_v1/'+date_of_name[file_i]+'.txt'
