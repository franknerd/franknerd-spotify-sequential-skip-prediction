...
pathlib.Path('output_1210').mkdir(parents=True, exist_ok=True)
...
for i in tqdm(range(start-1,end,1)):
    ...
    ''' import dataframe '''
    train = pd.read_csv(train_path[i],dtype=train_dtypes)
    X_train = train.loc[:, train.columns != 'skip_2']
    y_train = train['skip_2']

    X_test = pd.read_csv(test_path[i],dtype=test_dtypes)
    ...
    '''save file'''
    output_path = 'output_v1/'+date_of_name[i]+'.txt'
...
output_file = open('output_v1/submission.txt', 'w')
